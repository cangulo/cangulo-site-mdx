...
| `grep`  | `[COMMAND] \| grep -E [Regex]`    | Filters the output lines of `[COMMAND]` by the `[Regex]` BRRRRRCheck this  [cheatsheet for Regex](RegexCheatSheet.md)                                                       |
...
| `tee`   | `[COMMAND] \| tee [FILE]`         | Overwrites the `[FILE]` content with the `[COMMAND]` output (**stdout**).BRRRRRTo Include the **stderr** concatenate `&` after `\|`                                         |
| `tee`   | `[COMMAND] \| tee -a [FILE]`      | Appends the `[COMMAND]` output (**stdout**) to the `[FILE]` content.BRRRRRTo Include the **stderr** concatenate `&` after `\|`                                              |
| `vim`   | `vim [FILE]`                      | [Vim Cheatsheet](https://vim.rtorr.com)                                                                                                                                   |
| `vim`   | `[COMMAND] \| vim -`              | Sends `[COMMAND]` output directly into **vim**. To copy a line to the system clipboard you should:BRRRRR1. Go to the lineBRRRRR2. Type `:y+`                                  |
| `vim`   | copy a text line in the clipboard | 1. Verify you have vim-gnome and copyQ (try to install it both with `sudo apt-get install`BRRRRR2. Enter in vim BRRRRR3. Go to the lineBRRRRR4. Type `:y+` 5. Paste it anywhere |
...
| Example                | Remark                                                                                                                                                                                  |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `[COMMAND] > [FILE]`   | **Overwrites** the `[FILE]` content  with the `[COMMAND]` **stdout**. BRRRRRIt only works if the `[COMMAND]` is executed successfully                                                     |
| `[COMMAND] >> [FILE]`  | **Appends** the `[COMMAND]` **stdout** to the `[FILE]` content. BRRRRRIt only works if the `[COMMAND]` is executed successfully                                                           |
...
| `[COMMAND] 2> [FILE]`  | Display the `[COMMAND]` **stdout** in the terminal and overwrites the `[FILE]` content with the **stderr**.BRRRR Please note if there is no error the `[FILE]` content would result empty |
...
| `gedit`  | `io.elementary.code`                                                 | Open any file with the Elementary text editorBRRRRR**(Only works on Elementary OS)**                                     |
| `open_f` | `currentFolder=$(pwd);`BRRRRR`io.elementary.files $currentFolder -n &` | Open a new windows (`-n` of the Elementary file explorer in the current directoryBRRRRR**(Only works on Elementary OS)** |
